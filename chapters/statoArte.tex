...
Le informazioni del protocollo sono scritte con un linguaggio umano, quindi facile da testare e controllare in fase id implementazione.
\subsubsection{Stateless}
Il protocollo non mantiene lo stato della connessione, quindi se un utente fa due richieste allo stesso server, per quest'ultimo le due richieste sono indipendentemente l'une dalle altre. Questo potrebbe essere un punto a svavore sopratutto in caso di siti che richiedono un'autenticazione perché il client si deve autenticare ogni volta che accede. Per sopperire a questo problema vengono utilizzati i cookies, cioé delle stringhe che identificano il client e che vengono salvate sulla memoria del browser. Quando il client fa una richiesta, il cookie viene allegato alla richiesta e il server puó verificare tramite quello che il client é autenticato.
